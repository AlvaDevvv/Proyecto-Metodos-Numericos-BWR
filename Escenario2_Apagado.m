% =========================================================================
% Archivo: Escenario2_Apagado.m
% Descripción: Transitorio de Apagado (SCRAM). El reactor detiene la 
% generación de calor (q=0) y se enfría desde un estado muy caliente.
% =========================================================================
clear; clc; close all;
...
% 2. Sobrescritura Local de Parámetros (Escenario 2)
q_gen = 0;      % Cae a cero, el reactor se apagó
h_inf = 50000;  % El agua sigue fluyendo y enfriando
T_inf = 560;    
T_ini = 1500;   % Arranca desde un estado muy caliente
...
% 6. Gráficos
figure;
plot(r*1000, T_hist(:,1), '--k', 'DisplayName', 't = 0 s (Inicio)'); hold on;
...
plot(r*1000, T_hist(:, end), 'r', 'LineWidth', 1.5, 'DisplayName', ['t = ', num2str(t_final), ' s (Fin)']);

xline(R1*1000, '--', 'Color', [0.5 0.5 0.5], 'HandleVisibility','off');
xline(R2*1000, '--', 'Color', [0.5 0.5 0.5], 'HandleVisibility','off');
max_T = max(T_hist(:));
text(R1*1000/2, max_T*0.85, 'Pastilla UO_2', 'HorizontalAlignment','center');
text((R1+R2)*1000/2, max_T*0.95, 'Huelgo', 'HorizontalAlignment','center');
text((R2+R3)*1000/2, max_T*0.85, 'Vaina', 'HorizontalAlignment','center');

xlabel('Radio (mm)'); ylabel('Temperatura (K)');
title('Escenario 2: Transitorio de Apagado (Enfriamiento)');
